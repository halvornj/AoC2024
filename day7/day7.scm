...
(define (uint->bit-string uint length)
  (cond
    ((eq? 0 length) '())
...
(define (part1)
  (define (p1-iter equations acc)
    (cond
      ((null? equations) acc)
      ((is-valid-equation? (cons (car (car equations)) (reverse (cdr equations)))) (p1-iter (cdr equations) (+ acc (car (car equations))))) ;; car-car is first el in equation, target
